...
import { adventureList, adventurePlay } from './adventure.js';
import { classMaterial } from './classMaterial.js';
import { chatList, chatPlay } from './chatPartner.js';
import { roomList, roomPlay, roomSession } from './speakingRoom.js';
import { functionsLesson } from './functions.js';
import { classPrep } from './syllabus.js';
...
route('/adventure', (_p, v) => requireOnboard(() => adventureList(_p, v)));
route('/adventure/:id', (p, v) => requireOnboard(() => adventurePlay(p, v)));
route('/material', (_p, v) => requireOnboard(() => classMaterial(_p, v)));
route('/chat', (_p, v) => requireOnboard(() => chatList(_p, v)));
route('/chat/:id', (p, v) => requireOnboard(() => chatPlay(p, v)));
route('/room', (_p, v) => requireOnboard(() => roomList(_p, v)));
route('/room/session', (_p, v) => requireOnboard(() => roomSession(_p, v)));
route('/room/:id', (p, v) => requireOnboard(() => roomPlay(p, v)));
route('/functions', (_p, v) => requireOnboard(() => functionsLesson(_p, v)));
route('/prep', (_p, v) => requireOnboard(() => classPrep(_p, v)));
